...
//##// Submitted: 2023-01-23 04:27:16 +0000 UTC ( https://atcoder.jp/contests/typical90/submissions/38276665 ) 
...
let () =
  for i = 0 to (1 lsl mid) - 1 do
    let cnt1 = ref (Int 0) and cnt2 = ref 0 in
    for j = 0 to mid - 1 do
      if i land (1 lsl j) <> 0 then (
        cnt1 := !cnt1 +/ a.(j);
        cnt2 := !cnt2 + 1)
    done;
    if !cnt1 <=/ p then vec1.(!cnt2) <- !cnt1 :: vec1.(!cnt2)
  done

let () =
  for i = 0 to (1 lsl (n - mid)) - 1 do
    let cnt1 = ref (Int 0) and cnt2 = ref 0 in
    for j = 0 to n - mid - 1 do
      if i land (1 lsl j) <> 0 then (
        cnt1 := !cnt1 +/ a.(mid + j);
        cnt2 := !cnt2 + 1)
    done;
    if !cnt1 <=/ p then vec2.(!cnt2) <- !cnt1 :: vec2.(!cnt2)
  done
