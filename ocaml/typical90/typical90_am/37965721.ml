...
//##// Submitted: 2023-01-12 14:50:04 +0000 UTC ( https://atcoder.jp/contests/typical90/submissions/37965721 ) 
...
(* let rec cnt np p =
  Hashtbl.find_or_add cache (p, np) ~default:(fun () ->
      ...
      |> List.fold ~init:1 ~f:(fun acc x -> acc + if x = np then 0 else cnt p x)) *)
